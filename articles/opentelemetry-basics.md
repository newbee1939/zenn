...
この場合、システム全体で問題が発生すると原因特定が困難です。それぞれの監視ツールを一つずつチェックしてボトルネックを探すしかなく、膨大な時間がかかります。
...
これにより**ベンダーロックイン**が排除され、テレメトリーの送信先を自由に選択できます。
...
計装でアプリケーションから直接バックエンドに送信することも可能ですが、Collectorを挟むことが推奨されています。サービスが素早くCollectorにデータをオフロードし、Collector側で再試行、バッチ処理、暗号化、機密データのフィルタリングなどの追加処理を行えるためです。
