...
```mermaid
graph LR
  App["アプリケーション"] -->|"クエリ"| Pool["コネクション・プール"]
  Pool --- C1["コネクション 1"]
  Pool --- C2["コネクション 2"]
  Pool --- C3["コネクション N"]
  C1 --> DB[("データベース")]
...
```mermaid
graph LR
  subgraph CR["Cloud Run インスタンス"]
    subgraph App["アプリケーション（1 PrismaClient）"]
      subgraph Pool["コネクション・プール / connection_limit=4"]
        C1["コネクション 1"]
        C2["コネクション 2"]
        C3["コネクション 3"]
        C4["コネクション 4"]
...
```mermaid
graph LR
  Q1["クエリ 1"] --> C1["コネクション 1 - 使用中"]
  Q2["クエリ 2"] --> C2["コネクション 2 - 使用中"]
  Q3["クエリ 3"] -.->|"空きなし"| Queue["FIFO キュー\n（待ち状態）"]
  Q4["クエリ 4"] -.->|"空きなし"| Queue
  Q5["クエリ 5"] -.->|"空きなし"| Queue

  subgraph Pool["コネクション・プール（connection_limit=2）"]
    C1
    C2
  end
```
...
:::message
**Metrics Explorer の操作手順**
Google Cloud コンソール → [モニタリング] → [Metrics Explorer] を開きます。
PromQL または MQL を使用してクエリを作成できる画面が表示されます。
:::
...
:::message
**Prisma メトリクスの選択例**
[リソースの種類] に `prometheus_target` を選択し、[指標] に `prisma_client_queries_wait` などの Prisma 関連メトリクスを指定します。
:::

対象のCloud Runのリビジョンやインスタンスで絞ることで、以下のようにメトリクスの時間変化を可視化することができるでしょう。

```
prisma_pool_connections_open（メトリクス可視化イメージ）
...
```
prisma_client_queries_wait（チューニング前: connection_limit 不足）
...
```
prisma_client_queries_wait（チューニング後: connection_limit 増加）
